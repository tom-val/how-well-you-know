...
public class QuestionUserChoiceTests
{
    [Fact]
    ...
    {
        var (user, question) = CreateQuestion();
        var (_, question2) = CreateQuestion();

        var userChoice = QuestionUserChoice.Create(user, question, [question2.AnswerVariants.First(v => v.Notation == 'A')]);
    ...
    public void Create_GivenMultipleVariantsForNonMultipleAnswersQuestion_ReturnsError()
    {
        ...
        var userChoice = QuestionUserChoice.Create(user, question, [question.AnswerVariants.First(v => v.Notation == 'A'), question.AnswerVariants.First(v => v.Notation == 'B')]);
        userChoice.IsSuccess.Should().BeFalse();
